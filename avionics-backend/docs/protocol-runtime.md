...
`BuiltinProtocolFactory` 描述并创建两种内置协议，均可通过 `IProtocolFactory` 查询：

| 内部编号 | 名称 | 输入表示 `capture_representation` | 说明 |
|---|---|---|---|
| `0x00010001`（65537） | `word32-parity` | `captured_words` | 32 位大端字序列，每个字按奇校验校验，字标签非零 |
| `0x00010002`（65538） | `framed-crc16` | `wire_bytes` | 同步字加长度与 CRC 的分帧字节流 |

### word32-parity 字格式

每个字为 4 字节大端整数，位分配自高位到低位：标签 `label[31:24]`、`SDI[23:22]`、`数据 data[21:3]`、`SSM[2:1]`、`parity[0]`。整字置位数必须为奇数；`label` 为 0 视为非法。该字布局是面向字总线的常用排列，本实现只处理已捕获的字，不实现物理层或电气层。
...
- word32-parity：窗口内存在完整的 32 位字、所有字通过奇校验、标签非零，且累计不少于 4 个字时给出 `Strong` 候选并 `Identified`；字不足 4 个时为 `NeedMoreData`；出现结构不合法（长度非 4 的倍数、校验或标签失败）即不属于该协议，不产生候选。
